    ...
    <v-navigation-drawer v-model="navigationDrawer" :rail="navigationRail" permanent>
        ...
        <v-list-item title="Navigation" value="Navigation">
            ...
            <template v-slot:append>
                <v-btn v-if="!navigationRail" variant="text" icon="mdi-chevron-left"
                    @click.stop="handleNavigation(true)">
                </v-btn>

                <!-- open icon -->
                <v-btn v-if="navigationRail" variant="text" icon="mdi-chevron-right"
        ...
        <v-list density="compact" nav>
            <v-list-item v-for="item in navItems" link :key="item.title" :prepend-icon="item.icon"
                :title="$t(item.route)" :value="item.title" class="nav-link" @click="navigate(item)">
            </v-list-item>
        </v-list>
    </v-navigation-drawer>
</template>
