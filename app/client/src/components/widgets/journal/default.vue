<template>
  <v-row v-if="state && props.uuid" data-test-container="widgets/journal/default" :data-test-container-uuid=props.uuid>
    <v-col cols="12" md="6" lg="4" color="green">
      <div>
        <v-timeline side="end" align="end" class="mt-5">
          <template v-for="(entry, index) in data" :key="index">
            <v-timeline-item size="small" v-if="entry.category"
