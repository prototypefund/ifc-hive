<template>
  <v-container fluid class="pa-0" v-if="state">
    <v-row>
      <v-col cols="12" md="6" lg="4" color="green" style="overflow: hidden; width: 650px!important;x"
        :style="{ height: '1100px' }">
        <div style="height: 1100px!important; overflow-x: hidden; overflow-y: scroll; width: 850px">
          <v-timeline side="end" align="end" class="mt-5">
            <v-timeline-item v-for="(entry, index) in state.entries" :key="index" size="small"
              ...
              <div class="mt-2 mb-2">
                <v-chip label size="small" class="bg-grey-lighten-4" v-if="entry.locked > 1" dark
              ...
              <div style="width: 600px; margin-bottom: 80px">
                ...
                <v-alert v-if="entry.alert" :color="entry.alert.color" @click="setCurrent(entry)" class="mb-2">
                ...
                <div class="my-2" v-if="entry.tags && entry.tags.length > 0">
                  <v-chip label color="blue" v-for="tag in entry.tags" :key="tag" class="mr-2 mb-2">
                    {{ tag }}
                  </v-chip>
                </div>
                ...
                <v-card flat>
                  <v-card-text class="pa-0">
                    ...
                    <div class="mt-2" v-if="entry.content" style="padding-right: 50px">
                      ...
                      diam nonumy eirmod tempor invidunt <v-chip size="x-small" color="primary" @click=""># 14</v-chip>
                      ...
                      <v-chip label size="small" color="blue" dark>
                        ...
                        John Doe
                      </v-chip> ea rebum. Stet clita kasd gubergren, no
                ...
                <div class="my-2">
                  <v-chip label v-if="entry.images">
                    <v-icon class="mr-2">mdi-image-multiple-outline</v-icon> {{ entry.images }} Bilder
                  </v-chip>
                  <v-chip label v-if="entry.attachments">
                    <v-icon class="mr-2">mdi-attachment</v-icon> {{ entry.attachments }} Dokumente
                  </v-chip>
                </div>

                <div v-if="entry.request && entry.request.length > 0" class="ml-5 mb-4 mt-8">
                  <v-card v-for="request in entry.request" :key="entry.id + request.name" class="mb-2 pl-4" flat>
                    <v-card-text class="pa-0">

                      <v-chip label class="mr-4">
                        <v-icon :color="request.approved
                      ? 'success'
                      : request.approved === false
                        ? 'error'
                        : 'grey-lighten-1'">
                          {{ request.approved ? 'mdi-check' : request.approved === false ? 'mdi-close' : 'mdi-help-circle-outline' }}
                        </v-icon>
                      </v-chip>
